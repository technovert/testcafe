...
export default class Reporter {
  ...
  static _isSpecialStream(stream) {
    return (
      stream.isTTY || stream === process.stdout || stream === process.stderr
    );
  }
  ...
  static _createReportQueue(task) {
    ...
    return task.tests.map((test) =>
      Reporter._createReportItem(test, runsPerTest)
    );
  }
  ...
  async createErrorObject(errs) {
    let scriptErrors = errs.map((err) => {
      return this.plugin.formatError(err, "");
    });
    return scriptErrors;
  }
  ...
  async _shiftReportQueue(reportItem) {
    ...
    while (this.reportQueue.length && this.reportQueue[0].testRunInfo) {
      ...
      let scriptErrors = await this.createErrorObject(
        reportItem.testRunInfo.errs
      );
      let errors = await this.plugin.reportTestDone(
  ...
  async _resolveReportItem(reportItem, testRun) {
    if (this.task.screenshots.hasCapturedFor(testRun.test)) {
      reportItem.screenshotPath = this.task.screenshots.getPathFor(
        testRun.test
      );
      reportItem.screenshots = this.task.screenshots.getScreenshotsInfo(
        testRun.test
      );
    }
    ...
    if (testRun.quarantine) {
      reportItem.quarantine = testRun.quarantine.attempts.reduce(
        (result, errors, index) => {
    ...
    if (!reportItem.testRunInfo) {
      ...
      if (reportItem.test.skip) this.skipped++;
      else if (reportItem.errs.length) this.failed++;
      else this.passed++;
    }
  ...
  _prepareReportTestActionEventArgs({
    command,
    duration,
    result,
    testRun,
    ...
    if (typeof duration === "number") args.duration = duration;
  ...
  _assignTaskEventHandlers() {
    ...
    task.once("start", async () => {
      ...
      const userAgents = task.browserConnectionGroups.map(
        (group) => group[0].userAgent
      );
    ...
    task.on("test-run-done", async (testRun) => {
      const reportItem = this._getReportItemForTestRun(testRun);
      const isTestRunStoppedTaskExecution =
        !!testRun.errs.length && this.stopOnFirstFail;

      reportItem.pendingRuns = isTestRunStoppedTaskExecution
        ? 0
        : reportItem.pendingRuns - 1;
      reportItem.unstable = reportItem.unstable || testRun.unstable;
      reportItem.errs = reportItem.errs.concat(testRun.errs);
      reportItem.warnings = testRun.warningLog
        ? union(reportItem.warnings, testRun.warningLog.messages)
        : [];

      reportItem.browsers.push(
        Object.assign(
          { testRunId: testRun.id },
          getBrowser(testRun.browserConnection)
        )
      );
    ...
    task.once("done", async () => {
      ...
      await this.plugin.reportTaskDone(
        endTime,
        this.passed,
        task.warningLog.messages,
        result
      );
