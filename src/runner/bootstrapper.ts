...
function isReporterPluginFactory(
    value: string | Function
): value is ReporterPluginFactory {
...
type PromiseResult<T, E extends Error = Error> =
    | PromiseSuccess<T>
    | PromiseError<E>;

function isPromiseError<T, E extends Error = Error>(
    value: PromiseResult<T, E>
): value is PromiseError<E> {
...
export default class Bootstrapper {
    ...
    private static _splitBrowserInfo(
        browserInfo: BrowserInfoSource[]
    ): SeparatedBrowserInfo {
    ...
    private static async _hasLocalBrowsers(
        browserInfo: BrowserInfoSource[]
    ): Promise<boolean> {
        for (const browser of browserInfo) {
            if (browser instanceof BrowserConnection) continue;

            if (
                await browser.provider.isLocalBrowser(
                    void 0,
                    browser.browserName
                )
            )
    ...
    private static async _checkRequiredPermissions(
        browserInfo: BrowserInfoSource[]
    ): Promise<void> {
        const hasLocalBrowsers = await Bootstrapper._hasLocalBrowsers(
            browserInfo
        );
        ...
        if (hasLocalBrowsers) throw error;
    ...
    private _createAutomatedConnections(
    ...
    private async _getBrowserConnections(
        browserInfo: BrowserInfoSource[]
    ): Promise<BrowserSet> {
        const { automated, remotes } = Bootstrapper._splitBrowserInfo(
            browserInfo
        );
        ...
        browserConnections = browserConnections.concat(
            chunk(remotes, this.concurrency)
        );
    ...
    private async _compileTests({
        sourceList,
        compilerOptions,
    }: CompilerArguments): Promise<Test[]> {
        if (this.compilerService) {
            await this.compilerService.init();

            return this.compilerService.getTests({
                sourceList,
                compilerOptions,
            });
        }
    ...
    private async _getTests(): Promise<Test[]> {
        ...
        const {
            sourceList,
            compilerOptions,
        } = await this._getCompilerArguments(cwd);

        if (!sourceList.length)
            throw new GeneralError(
                RUNTIME_ERRORS.testFilesNotFound,
                getConcatenatedValuesString(this.sources, "\n", ""),
                cwd
            );
        ...
        if (this.filter) tests = this._filterTests(tests, this.filter);
    ...
    private async _getCompilerArguments(
        cwd: string
    ): Promise<CompilerArguments> {
    ...
    private async _ensureOutStream(
        outStream: string | WritableStream
    ...
    private _requireReporterPluginFactory(
        reporterName: string
    ): ReporterPluginFactory {
        try {
            return require("../reporter/reporter-plugin");
        } catch (err) {
            throw new GeneralError(
    ...
    private _getPluginFactory(
        reporterFactorySource: string | ReporterPluginFactory
    ): ReporterPluginFactory {
    ...
    private async _getReporterPlugins(): Promise<ReporterPluginSource[]> {
        ...
        return Promise.all(
            this.reporters.map(async ({ name, output }) => {
                const pluginFactory = this._getPluginFactory(name);
                const outStream = output
                    ? await this._ensureOutStream(output)
                    : void 0;
    ...
    private async _startTestedApp(): Promise<TestedApp | undefined> {
        if (!this.appCommand) return void 0;
    ...
    private _wrapBootstrappingPromise<T>(
        promise: Promise<T>
    ...
    private async _getBootstrappingError(
        browserSetStatus: PromiseResult<BrowserSet>,
        testsStatus: PromiseResult<Test[]>,
        testedAppStatus: PromiseResult<TestedApp | undefined>
    ): Promise<Error> {
        if (!isPromiseError(browserSetStatus))
            await browserSetStatus.result.dispose();

        if (
            !isPromiseError(browserSetStatus) &&
            !isPromiseError(testedAppStatus) &&
            testedAppStatus.result
        )
            await testedAppStatus.result.kill();

        if (isPromiseError(testsStatus)) return testsStatus.error;

        if (isPromiseError(testedAppStatus)) return testedAppStatus.error;

        if (isPromiseError(browserSetStatus)) return browserSetStatus.error;

        return new Error("Unexpected call");
    }

    private _getBootstrappingPromises<T>(
        arg: PromiseCollection<T>
    ): PromiseCollection<ResultCollection<T>> {
        const result = ({} as unknown) as PromiseCollection<
            ResultCollection<T>
        >;

        for (const k in arg) result[k] = this._wrapBootstrappingPromise(arg[k]);

        return result;
    }

    private async _bootstrapParallel(
        browserInfo: BrowserInfoSource[]
    ): Promise<BasicRuntimeResources> {
        ...
        const bootstrappingResultPromises = this._getBootstrappingPromises(
            bootstrappingPromises
        );
        ...
        if (
            isPromiseError(browserSetResults) ||
            isPromiseError(testResults) ||
            isPromiseError(appResults)
        )
    ...
    public async createRunnableConfiguration(): Promise<RuntimeResources> {
        ...
        if (OS.mac) await Bootstrapper._checkRequiredPermissions(browserInfo);

        if (await this._canUseParallelBootstrapping(browserInfo))
            return {
                reporterPlugins,
                ...(await this._bootstrapParallel(browserInfo)),
                commonClientScripts,
            };

        return {
            reporterPlugins,
            ...(await this._bootstrapSequence(browserInfo)),
            commonClientScripts,
        };
    }
}
