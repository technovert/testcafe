...
export default class Runner extends EventEmitter {
  constructor(proxy, browserConnectionGateway, configuration, compilerService) {
    ...
    this.proxy = proxy;
    this.bootstrapper = this._createBootstrapper(
  ...
  _disposeReporters(reporters) {
    return Promise.all(
      reporters.map((reporter) =>
        reporter.dispose().catch((e) => DEBUG_LOGGER(e))
      )
    );
  }
  ...
  _disposeTestedApp(testedApp) {
    return testedApp
      ? testedApp.kill().catch((e) => DEBUG_LOGGER(e))
      : Promise.resolve();
  }
  ...
  async _getTaskResult(task, browserSet, reporters, testedApp) {
    if (!task.opts.live) {
      task.on("browser-job-done", (job) => {
        job.browserConnections.forEach((bc) =>
          browserSet.releaseConnection(bc)
        );
      });
    }
    ...
    const taskDonePromise = task
      .once("done")
      .then(() => browserSetErrorPromise.cancel())
      .then(() => {
        return Promise.all(
          reporters.map((reporter) => reporter.pendingTaskDonePromise)
        );
      });
    ...
    try {
      ...
      await this._disposeTaskAndRelatedAssets(
        task,
        browserSet,
        reporters,
        testedApp
  ...
  _runTask(reporterPlugins, browserSet, tests, testedApp) {
    const task = this._createTask(
      tests,
      browserSet.browserConnectionGroups,
      this.proxy,
      this.configuration.getOptions()
    );
    const reporters = reporterPlugins.map(
      (reporter) =>
        new Reporter(reporter.plugin, task, reporter.outStream, reporter.name)
    );
    const completionPromise = this._getTaskResult(
      task,
      browserSet,
      reporters,
      testedApp
    ...
    completionPromise.then(onTaskCompleted).catch(onTaskCompleted);
  ...
  _validateDebugLogger() {
    ...
    const debugLoggerDefinedCorrectly =
      debugLogger === null ||
      (!!debugLogger &&
        ["showBreakpoint", "hideBreakpoint"].every(
          (method) => method in debugLogger && isFunction(debugLogger[method])
        ));
  ...
  _getScreenshotOptions() {
    let { path, pathPattern } =
      this.configuration.getOption(OPTION_NAMES.screenshots) || {};

    if (!path) path = this.configuration.getOption(OPTION_NAMES.screenshotPath);

    if (!pathPattern)
      pathPattern = this.configuration.getOption(
        OPTION_NAMES.screenshotPathPattern
      );
  ...
  _validateScreenshotOptions() {
    ...
    const disableScreenshots =
      this.configuration.getOption(OPTION_NAMES.disableScreenshots) || !path;

    this.configuration.mergeOptions({
      [OPTION_NAMES.disableScreenshots]: disableScreenshots,
    });
    ...
    if (path) {
      ...
      this.configuration.mergeOptions({
        [OPTION_NAMES.screenshots]: { path: resolvePath(path) },
      });
    }
    ...
    if (pathPattern) {
      ...
      this.configuration.mergeOptions({
        [OPTION_NAMES.screenshots]: { pathPattern },
      });
    }
  }
  ...
  async _validateVideoOptions() {
    ...
    const videoEncodingOptions = this.configuration.getOption(
      OPTION_NAMES.videoEncodingOptions
    );
    ...
    this.configuration.mergeOptions({
      [OPTION_NAMES.videoPath]: resolvePath(videoPath),
    });
    ...
    if (!videoOptions) {
      ...
      this.configuration.mergeOptions({
        [OPTION_NAMES.videoOptions]: videoOptions,
      });
    }

    if (videoOptions.ffmpegPath)
      videoOptions.ffmpegPath = resolvePath(videoOptions.ffmpegPath);
    else videoOptions.ffmpegPath = await detectFFMPEG();
  ...
  _validateBrowsersForAllowMultipleWindowsOption(browserSet) {
    const browserConnections = browserSet.browserConnectionGroups.map(
      (browserConnectionGroup) => browserConnectionGroup[0]
    );
    const unsupportedBrowserConnections = browserConnections.filter(
      (browserConnection) => !browserConnection.activeWindowId
    );

    if (!unsupportedBrowserConnections.length) return;

    const unsupportedBrowserAliases = unsupportedBrowserConnections.map(
      (browserConnection) => browserConnection.browserInfo.alias
    );
    const browserAliases = getConcatenatedValuesString(
      unsupportedBrowserAliases
    );
  ...
  _validateAllowMultipleWindowsOption(tests, browserSet) {
    const allowMultipleWindows = this.configuration.getOption(
  ...
  _validateScreenshotPath(screenshotPath, pathType) {
    ...
    if (forbiddenCharsList.length)
      throw new GeneralError(
        ...
        screenshotPath,
        pathType,
        renderForbiddenCharsList(forbiddenCharsList)
      );
  }
  ...
  _setBootstrapperOptions() {
    ...
    this.bootstrapper.sources =
      this.configuration.getOption(OPTION_NAMES.src) ||
      this.bootstrapper.sources;
    this.bootstrapper.browsers =
      this.configuration.getOption(OPTION_NAMES.browsers) ||
      this.bootstrapper.browsers;
    this.bootstrapper.concurrency = this.configuration.getOption(
      OPTION_NAMES.concurrency
    );
    this.bootstrapper.appCommand =
      this.configuration.getOption(OPTION_NAMES.appCommand) ||
      this.bootstrapper.appCommand;
    this.bootstrapper.appInitDelay = this.configuration.getOption(
      OPTION_NAMES.appInitDelay
    );
    this.bootstrapper.filter =
      this.configuration.getOption(OPTION_NAMES.filter) ||
      this.bootstrapper.filter;
    this.bootstrapper.reporters =
      this.configuration.getOption(OPTION_NAMES.reporter) ||
      this.bootstrapper.reporters;
    this.bootstrapper.tsConfigPath = this.configuration.getOption(
      OPTION_NAMES.tsConfigPath
    );
    this.bootstrapper.clientScripts =
      this.configuration.getOption(OPTION_NAMES.clientScripts) ||
      this.bootstrapper.clientScripts;
    this.bootstrapper.allowMultipleWindows = this.configuration.getOption(
      OPTION_NAMES.allowMultipleWindows
    );
    this.bootstrapper.disablePageReloads = this.configuration.getOption(
      OPTION_NAMES.disablePageReloads
    );
    this.bootstrapper.skipJsErrors = this.configuration.getOption(
  ...
  screenshots(...options) {
    ...
    this.configuration.mergeOptions({
      screenshots: { path, takeOnFails, pathPattern, fullPage },
    });
  ...
  async _prepareClientScripts(tests, clientScripts) {
    return Promise.all(
      tests.map(async (test) => {
        if (test.isLegacy) return;

        let loadedTestClientScripts = await loadClientScripts(
          test.clientScripts,
          dirname(test.testFile.filename)
        );
  ...
  async stop() {
    ...
    const cancellationPromises = mapReverse(
      this.pendingTaskPromises,
      (taskPromise) => taskPromise.cancel()
    );
