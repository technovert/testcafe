...
ReadyTest now watches source files and reruns
...
`,
            ...
            fileWatchingEnabled:
                "ReadyTest is watching the source files. Save the changes to run tests.",
            fileWatchingDisabled: "ReadyTest is not watching the source files.",
            nothingToStop: "There are no tests running at the moment.",
            testCafeStopping: "Stopping ReadyTest live mode...",
    ...
    writeRunTestsMessage(sourcesChanged) {
        const statusMessage = sourcesChanged
            ? this.MESSAGES.sourceChanged
            : this.MESSAGES.testRunStarting;

        this._write(statusMessage);
    }

    writeTestsFinishedMessage() {
        const statusMessage = this.watching
            ? this.MESSAGES.testRunFinishedWatching
            : this.MESSAGES.testRunFinishedNotWatching;
    ...
    writeToggleWatchingMessage(enable) {
        ...
        const statusMessage = enable
            ? this.MESSAGES.fileWatchingEnabled
            : this.MESSAGES.fileWatchingDisabled;
