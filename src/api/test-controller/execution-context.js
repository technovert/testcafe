...
function createRequire(filename) {
  ...
  if (Module.createRequire) return Module.createRequire(filename);
...
export function createExecutionContext(testRun) {
  ...
  return createContext(
    new Proxy(replacers, {
      get: (target, property) => {
        if (replacers.hasOwnProperty(property)) return replacers[property];

        if (global.hasOwnProperty(property)) return global[property];
