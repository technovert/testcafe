...
export class AssertionExecutor extends EventEmitter {
  constructor(command, timeout, callsite) {
    ...
    if (actualCommand instanceof ReExecutablePromise)
      ...
      !this.command.options.allowUnawaitedPromise &&
      isThennable(actualCommand)
    )
  ...
  async run() {
    ...
    try {
      ...
      if (err.isTestCafeError) err.callsite = this.callsite;
