...
const exportableLib = require("./api/exportable-lib");
...
async function getValidPort(port) {
  if (port) {
    ...
    if (!isFree) throw new GeneralError(RUNTIME_ERRORS.portIsNotFree, port);
...
Object.keys(exportableLib).forEach((key) => {
  Object.defineProperty(createTestCafe, key, {
    get: () => exportableLib[key],
  });
});
