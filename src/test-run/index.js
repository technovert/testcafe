...
export default class TestRun extends AsyncEventEmitter {
  ...
  _shouldResolveCurrentDriverTask(driverStatus) {
    ...
    const isExecutingObservationCommand =
      currentCommand instanceof observationCommands.ExecuteSelectorCommand ||
  ...
  _adjustConfigurationWithCommand(command) {
    if (command.type === COMMAND_TYPE.testDone) {
      ...
      if (this.debugLogger) this.debugLogger.hideBreakpoint(this.session.id);
  ...
  async _getStateSnapshotFromRole(role) {
    ...
    if (role.phase === ROLE_PHASE.uninitialized) await role.initialize(this);
