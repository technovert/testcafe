...
export default class CLIArgumentParser {
    ...
    private _describeProgram(): void {
        const version = JSON.parse(read("../../package.json") as string)
            .version;

        this.program
            .version(version, "-v, --version")
            .usage(
                "[options] <comma-separated-browser-list> <file-or-glob ...>"
            )
            .description(CLIArgumentParser._getDescription())

            .option(
                "-b, --list-browsers [provider]",
                "output the aliases for local browsers or browsers available through the specified browser provider"
            )
            .option(
                "-r, --reporter <name[:outputFile][,...]>",
                "specify the reporters and optionally files where reports are saved"
            )
            .option(
                "-s, --screenshots <option=value[,...]>",
                "specify screenshot options"
            )
            .option(
            ...
            .option(
                "-p, --screenshot-path-pattern <pattern>",
                "use patterns to compose screenshot file names and paths: ${BROWSER}, ${BROWSER_VERSION}, ${OS}, etc."
            )
            .option("-q, --quarantine-mode", "enable the quarantine mode")
            .option(
                "-d, --debug-mode",
                "execute test steps one by one pausing the test after each step"
            )
            .option(
                "-e, --skip-js-errors",
                "make tests not fail when a JS error happens on a page"
            )
            .option(
                "-u, --skip-uncaught-errors",
                "ignore uncaught errors and unhandled promise rejections, which occur during test execution"
            )
            .option(
                "-t, --test <name>",
            ...
            .option(
                "-f, --fixture <name>",
                "run only fixtures with the specified name"
            )
            .option(
                "-F, --fixture-grep <pattern>",
                "run only fixtures matching the specified pattern"
            )
            .option(
                "-a, --app <command>",
                "launch the tested app using the specified command before running tests"
            )
            .option("-c, --concurrency <number>", "run tests concurrently")
            .option(
                "-L, --live",
                "enable live mode. In this mode, ReadyTest watches for changes you make in the test files. These changes immediately restart the tests so that you can see the effect."
            )
            .option(
                "--test-meta <key=value[,key2=value2,...]>",
                "run only tests with matching metadata"
            )
            .option(
                "--fixture-meta <key=value[,key2=value2,...]>",
                "run only fixtures with matching metadata"
            )
            .option("--debug-on-fail", "pause the test if it fails")
            .option(
                "--app-init-delay <ms>",
                "specify how much time it takes for the tested app to initialize"
            )
            .option(
                "--selector-timeout <ms>",
                "specify the time within which selectors make attempts to obtain a node to be returned"
            )
            .option(
                "--assertion-timeout <ms>",
                "specify the time within which assertion should pass"
            )
            .option(
                "--page-load-timeout <ms>",
                "specify the time within which ReadyTest waits for the `window.load` event to fire on page load before proceeding to the next test action"
            )
            .option(
                "--speed <factor>",
                "set the speed of test execution (0.01 ... 1)"
            )
            ...
            .option(
                "--proxy-bypass <rules>",
                "specify a comma-separated list of rules that define URLs accessed bypassing the proxy server"
            )
            .option(
                "--ssl <options>",
                "specify SSL options to run ReadyTest proxy server over the HTTPS protocol"
            )
            .option("--video <path>", "record videos of test runs")
            .option(
                "--video-options <option=value[,...]>",
                "specify video recording options"
            )
            .option(
                "--video-encoding-options <option=value[,...]>",
                "specify encoding options"
            )
            .option("--dev", "enables mechanisms to log and diagnose errors")
            .option(
                "--qr-code",
                "outputs QR-code that repeats URLs used to connect the remote browsers"
            )
            .option(
                "--sf, --stop-on-first-fail",
                "stop an entire test run if any test fails"
            )
            .option(
                "--ts-config-path <path>",
                "use a custom TypeScript configuration file and specify its location"
            )
            .option(
                "--cs, --client-scripts <paths>",
            ...
            .option(
                ...
                "disable page caching during test execution"
            )
            .option(
                "--disable-page-reloads",
                "disable page reloads between tests"
            )
        ...
        this.experimental
            .allowUnknownOption()
            .option(
                "-m, --allow-multiple-windows",
                "run ReadyTest in the multiple windows mode"
            )
            .option(
                "--experimental-compiler-service",
    ...
    public async _parseFilteringOptions(): Promise<void> {
        if (this.opts.testGrep)
            this.opts.testGrep = getGrepOptions(
                "--test-grep",
                this.opts.testGrep as string
            );

        if (this.opts.fixtureGrep)
            this.opts.fixtureGrep = getGrepOptions(
                "--fixture-grep",
                this.opts.fixtureGrep as string
            );

        if (this.opts.testMeta)
            this.opts.testMeta = await getMetaOptions(
                "--test-meta",
                this.opts.testMeta as string
            );

        if (this.opts.fixtureMeta)
            this.opts.fixtureMeta = await getMetaOptions(
                "--fixture-meta",
                this.opts.fixtureMeta as string
            );
    ...
    private _parseAppInitDelay(): void {
        if (this.opts.appInitDelay) {
            assertType(
                ...
                null,
                "Tested app initialization delay",
                this.opts.appInitDelay
            );

            this.opts.appInitDelay = parseInt(
                this.opts.appInitDelay as string,
                10
            );
        }
    }
    ...
    private _parseSelectorTimeout(): void {
        if (this.opts.selectorTimeout) {
            assertType(
                ...
                null,
                "Selector timeout",
                this.opts.selectorTimeout
            );

            this.opts.selectorTimeout = parseInt(
                this.opts.selectorTimeout as string,
                10
            );
        }
    }
    ...
    private _parseAssertionTimeout(): void {
        if (this.opts.assertionTimeout) {
            ...
            this.opts.assertionTimeout = parseInt(
                this.opts.assertionTimeout as string,
                10
            );
        }
    }
    ...
    private _parsePageLoadTimeout(): void {
        if (this.opts.pageLoadTimeout) {
            assertType(
                ...
                null,
                "Page load timeout",
                this.opts.pageLoadTimeout
            );

            this.opts.pageLoadTimeout = parseInt(
                this.opts.pageLoadTimeout as string,
                10
            );
        }
    }
    ...
    private _parseConcurrency(): void {
        if (this.opts.concurrency)
            this.opts.concurrency = parseInt(
                this.opts.concurrency as string,
                10
            );
    }
    ...
    private _parsePorts(): void {
        if (this.opts.ports) {
            const parsedPorts = (this.opts
                .ports as string) /* eslint-disable-line no-extra-parens */
                .split(",")
    ...
    private _parseBrowsersFromArgs(): void {
        ...
        this.opts.browsers = splitQuotedText(browsersArg, ",").filter(
            (browser) => browser && this._checkAndCountRemotes(browser)
        );
    }
    ...
    private async _parseReporters(): Promise<void> {
        const reporters = this.opts.reporter
            ? (this.opts.reporter as string).split(",")
    ...
    private async _parseScreenshotOptions(): Promise<void> {
        if (this.opts.screenshots)
            this.opts.screenshots = await getScreenshotOptions(
                this.opts.screenshots
            );
        else this.opts.screenshots = {};

        if (
            !has(this.opts.screenshots, SCREENSHOT_OPTION_NAMES.pathPattern) &&
            this.opts.screenshotPathPattern
        )
            this.opts.screenshots[
                SCREENSHOT_OPTION_NAMES.pathPattern
            ] = this.opts.screenshotPathPattern;

        if (
            !has(this.opts.screenshots, SCREENSHOT_OPTION_NAMES.takeOnFails) &&
            this.opts.screenshotsOnFails
        )
            this.opts.screenshots[
                SCREENSHOT_OPTION_NAMES.takeOnFails
            ] = this.opts.screenshotsOnFails;
    }

    private async _parseVideoOptions(): Promise<void> {
        if (this.opts.videoOptions)
            this.opts.videoOptions = await getVideoOptions(
                this.opts.videoOptions as string
            );

        if (this.opts.videoEncodingOptions)
            this.opts.videoEncodingOptions = await getVideoOptions(
                this.opts.videoEncodingOptions as string
            );
    }
    ...
    private _parseListBrowsers(): void {
        ...
        if (!this.opts.listBrowsers) return;

        this.opts.providerName =
            typeof listBrowserOption === "string"
                ? listBrowserOption
                : "locally-installed";
    }
