...
export default class CLIArgumentParser {
    ...
    private _describeProgram (): void {
        ...
        this.program
            ...
            .option('-L, --live', 'enable live mode. In this mode, ReadyTest watches for changes you make in the test files. These changes immediately restart the tests so that you can see the effect.')
            ...
            .option('--page-load-timeout <ms>', 'specify the time within which ReadyTest waits for the `window.load` event to fire on page load before proceeding to the next test action')
            ...
            .option('--ssl <options>', 'specify SSL options to run ReadyTest proxy server over the HTTPS protocol')
        ...
        this.experimental
            .allowUnknownOption()
            .option('-m, --allow-multiple-windows', 'run ReadyTest in the multiple windows mode')
